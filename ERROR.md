...
- **Status:** User's card has issues — payment method path unavailable. **Workaround adopted:** local gates (`npm run lint` + `npm run build` frontend, `python -m compileall app` backend) are the mandatory gate of record; CI workflow stays in repo and is expected to go green once the restriction lifts (support ticket / account establishment). Optional upgrade path: self-hosted runner on the dev PC (no GitHub billing involved).
